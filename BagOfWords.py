...
query_portal = "SELECT folioSAC, nomDependencia, dependencia_id, descripcion " \
               "FROM portal, peticion, dependencia " \
               "WHERE dependenciaId=dependencia_id and folioSAC=folioPeticion"

# query_portal = "SELECT folioSAC,tema_id,CONCAT_WS('_',nomTema,nomsubtema)
# as nomTema_nomsubtema,descripcion
# FROM portal,peticion,temas WHERE folioSAC=folioPeticion AND temaId=tema_id"
all_portal = sql.read_sql(query_portal, db_conn).dropna()
query_temas = "SELECT dependencia_id FROM peticion " \
              "GROUP BY dependencia_id ORDER BY dependencia_id"
        ...
        for f in num_features:
            for e in num_estimators:
                ...
                for i in xrange(0, len(test["descripcion"])):
                    ...
                    clean_test_descripcions.append(u" ".join(KaggleWord2VecUtility.review_to_wordlist(clean_text_test,
                                                                                                      True)))
                ...
                output = pd.DataFrame(data={"folioSAC": test["folioSAC"],
                                            "dependencia_id": result,
                                            "test_id": test["dependencia_id"],
                                            "nomDependencia": test["nomDependencia"]})

                # Use pandas to write the comma-separated output file
                file_name = 'Bag_of_Words_model_ng{ng}_nf{nf}_ne{ne}.csv'.format(ng=g, nf=f, ne=e)
                output.to_csv(os.path.join(os.path.dirname(__file__), 'data', file_name),
                              index=False,
                              quoting=2,
                              encoding='utf-8')
